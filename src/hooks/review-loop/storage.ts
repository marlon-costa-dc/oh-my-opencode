...
export function readState(directory: string, customPath?: string): ReviewLoopState | null {
  ...
  try {
    ...
    const targetBranch = data.target_branch
    const prFiles = data.pr_files

    if (active === undefined || iteration === undefined || targetBranch === undefined) {
      return null
    }
    ...
    const prFilesArray = Array.isArray(prFiles) ? (prFiles as string[]) : []
    ...
    return {
      ...
      target_branch: stripQuotes(targetBranch),
      pr_files: prFilesArray,
    }
  } catch {
    return null
  }
...
export function writeState(
  ...
  try {
    ...
    const prFilesYaml = state.pr_files.length > 0
      ? `pr_files:\n${state.pr_files.map((f) => `  - ${f}`).join("\n")}\n`
      : "pr_files: []\n"
